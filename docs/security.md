...
| CORS attacks                               | Lambda-owned `Access-Control-Allow-Origin` allowlist from `CORS_ALLOW_ORIGIN` env (no wildcard); gateway CORS disabled (`cors: false`) so it can't override it | `packages/functions/src/lib/respond.ts`, `sst.config.ts`                          |
