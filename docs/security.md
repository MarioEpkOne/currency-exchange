...
| CORS attacks                               | Gateway-managed `Access-Control-Allow-Origin` allowlist (deployed site origin + localhost), never a wildcard (an HTTP API strips Lambda-set CORS headers, so CORS must live on the gateway) | `sst.config.ts`                                                                   |
